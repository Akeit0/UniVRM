...
namespace UniJSON
{
    public static class Utf8StringExtensions
    {
        ...
        public static bool TrySearchByte(this Utf8String src, Byte target, out int pos)
        {
            var bs = src.AsSpan();
        ...
        internal static bool TrySearchUnWhiteSpace(this Utf8String src, out int pos)
        {
            var bs = src.AsSpan();
            for (var index = 0; index < bs.Length; index++)
            {
        ...
        internal static bool TrySearchDoubleQuote(this Utf8String src, int start, out int pos)
        {
            pos = -1;
            var bs = src.AsSpan()[start..];
            for (var index = 0; index < bs.Length; index++)
            {
                var b = bs[index];
                if (b == '\\')
                {
                    ...
                    switch ((char)next)
                    {
                        ...
                        default:
                            // unknown escape
                            throw new ParserException("unknown escape: " + (char)next);
                    }
                }
                else if (b == '"')
                {
